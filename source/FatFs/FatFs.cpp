 ...
 *
 * Modified by Manuel Iglesias to work with NXP FatFs port of Elm ChaN, June 2016
 *
...
int FileFs::open( char * fileName, uint8_t mode )
{
  ...
  if(j >= MAX_FILES)
	  return -1;		// cannot open more files
  ffs_result = f_open( & ffile[j], fileName0, mode );
  if(ffs_result == FR_OK){
	  _open[j] = true;
	  return j;
  }
...
bool FileFs::close(int hlr)
{
  ffs_result =  f_close( & ffile[hlr] );
  _open[hlr] = false;
  return ffs_result == FR_OK;
}
...
uint32_t FileFs::write(int hlr, void * buf, uint32_t lbuf )
{
  uint32_t nwrt;

  ffs_result = f_write( & ffile[hlr], buf, (UINT)lbuf, (UINT*)&nwrt );
  return nwrt;
}
...
int FileFs::writeString( int hlr, char * str )
{
  return f_puts( str, & ffile[hlr] );
}
...
bool FileFs::writeChar( int hlr, char car )
{
  return f_putc( car, & ffile[hlr] ) == 1;
}
...
uint32_t FileFs::read( int hlr, void * buf, uint32_t lbuf )
{
  uint32_t nrd;

  ffs_result = f_read( & ffile[hlr], buf, (UINT)lbuf, (UINT*)&nrd );
  return nrd;
}
...
int16_t FileFs::readString( int hlr, char * str, int len )
{
  str = f_gets( str, len, & ffile[hlr] );
  if( str == NULL )
    return -1;
...
char FileFs::readChar(int hlr)
{
  char     car;
  uint32_t nrd;

  ffs_result = f_read( & ffile[hlr], & car, 1, (UINT*)& nrd );
  return nrd == 1 ? car : -1;
}
...
uint16_t FileFs::readInt(int hlr)
{
  uint16_t i = 0;
  char c;
...
uint16_t FileFs::readHex(int hlr)
{
  uint16_t i = 0;
  char c;
...
uint32_t FileFs::curPosition(int hlr)
{
  return f_tell( & ffile[hlr] );
}
...
bool FileFs::seekSet( int hlr, uint32_t cur )
{
  ffs_result = f_lseek( & ffile[hlr], cur );
  return ffs_result == FR_OK;
}

// Return size of file

uint32_t FileFs::fileSize(int hlr)
{
  return f_size(&ffile[hlr]);
}
