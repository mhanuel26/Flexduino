...
void WebSocketsServer::clientDisconnect(WSclient_t * client) {
	...
	if(client->tcp->connected()) {
		client->tcp->flush();			// this seems like necessary on winc1500 due to bug found. See git issue #70
		client->tcp->stop();
	}
	delete client->tcp;
	client->tcp = NULL;
...
// * @param client
// */
bool WebSocketsServer::newClient(WEBSOCKETS_NETWORK_CLIENT_CLASS * TCPclient) {
	WSclient_t * client = &_clients[0];
	WEBSOCKETS_NETWORK_CLIENT_CLASS	**clientIdx = WiFi._client;
	uint8_t i;

    i = TCPclient->getSockNum();
	client += i;
	clientIdx += i;
	// we got the client number in index i
	if(client->tcp == NULL){
		// we got a new connection, but verify
		if(client->status == WSC_NOT_CONNECTED){
			client->num = i;
			client->status = WSC_HEADER;
			client->tcp = *clientIdx;
...
void WebSocketsServer::process(void) {
    ...
    tmp = server_.mavailable();			// my own available ... pufff arduino bugs and tricks
    if(tmp != NULL){
