 ...
 * @param headerToPayload bool  set true if the payload has reserved 14 Byte at the beginning to dynamically add the Header (payload need to be in RAM!)
 */
void WebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool mask, bool fin, bool headerToPayload) {
    ...
    if(!headerToPayload && ((length > 0) && (length < 1400))) {
        DEBUG_WEBSOCKETS("[WS][%d][sendFrame] pack to one TCP package...\r\n", client->num);
        uint8_t * dataPtr = (uint8_t *) malloc(length + WEBSOCKETS_MAX_HEADER_SIZE);
        if(dataPtr) {
            memcpy((dataPtr + WEBSOCKETS_MAX_HEADER_SIZE), payload, length);
...
#else
    DEBUG_WEBSOCKETS("[WS][%d][sendFrame] sending Frame Done.\r\n");
#endif
